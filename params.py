...
parser.add_argument('--config', default='configs/procc/utzappos.yml', help='path of the config file (training only)')
...
parser.add_argument('--logpath', default='logs/bims/utzappos/', help='Path to dir where to logs are stored (test only)')
...
parser.add_argument('--load', default='', help='path to checkpoint to load from')
parser.add_argument('--image_extractor', default = 'resnet18_s', help = 'Feature extractor model')
...
parser.add_argument('--open_world', action='store_true', default=1, help='perform open world experiment')
parser.add_argument('--partial',action='store_true', default=0, help='Partial Label Setting')
parser.add_argument('--gumbel',action='store_true', help='Use Gumbel-Softmax')
parser.add_argument('--test_batch_size', type=int, default=256, help="Batch size at test/eval time")
parser.add_argument('--fast', action='store_true', default=True, help='Fast version of evaluator (for the open world)')
parser.add_argument('--closed', action='store_true', default=False, help='Fast version of evaluator (for the closed world)')
...
parser.add_argument('--update_features', action = 'store_true', default=0, help='If specified, train feature extractor')
parser.add_argument('--freeze_features', action = 'store_true', default=0, help='If specified, put extractor in eval mode')
parser.add_argument('--emb_init', default=True, help='w2v|ft|gl|glove|word2vec|fasttext, name of embeddings to use for initializing the primitives')
...
# parser.add_argument('--hard_masking', action='store_true', default=0, help='hard masking during test time')
